...
count_data <-  data |>
  ...
  mutate(log_desertion_strictness = factor(log_desertion_strictness),
         tick = (tick+1)/2)

count_summary <- count_data |>
  group_by(tick, log_desertion_strictness) |>
...
ggplot() +
  geom_line(data = count_data, 
            aes(x = tick, y = count_eas, 
                color = log_desertion_strictness, 
                group = interaction(combo_id, run_id, log_desertion_strictness)), 
            alpha = 0.2) +
  geom_ribbon(data = count_summary, 
              aes(x = tick, 
                  ymin = ribbon_lower, 
                  ymax = ribbon_upper, 
                  fill = log_desertion_strictness), 
              alpha = 0.3) +
  geom_line(data = count_summary, 
            aes(x = tick, y = mean_ea_count, color = log_desertion_strictness), 
            linewidth = 1.2) +
  ...
  scale_color_brewer(type = "qual", palette = "Spectral", name = "Desertion Strictness") +
  scale_fill_brewer(type = "qual", palette = "Spectral", name = "Desertion Strictness") +
  labs(x = "Years", 
       y = "Number of EAs",
       caption = cap_text,
       title = "Number of EAs over time by Desertion Strictness") +
...
ggsave("./figures/leave_strictness_number_of_eas.png",
...
ggplot() +
  ...
  geom_line(data = grad_count_summary, 
            aes(x = tick, y = mean_ea_year4, color = log_desertion_strictness), 
            linewidth = 1.2) +
  ...
  labs(x = "Years (Semester 2 only)", 
       ...
       caption = cap_text,
       title = "Number of EAs Graduating at the end of each year by Desertion Strictness") +
  theme(panel.background = element_rect(fill = "white", color = "black"),
        panel.grid.major.y = element_line(color = "grey"),
        panel.grid.minor.y = element_line(color = "lightgrey"),
        panel.grid.minor.x = element_line(color = NA),          
...
ggsave("./figures/leave_strictness_number_of_ea_grads.png",
...
df_counts <- exp_data |>
  select(combo_id, run_id, tick, log_desertion_strictness, count_eas, count_noneas) |>
  pivot_longer(
...
# Join the datasets
exp_data <- df_long |>
  left_join(df_counts, by = c("combo_id", "run_id", "tick","log_desertion_strictness","type")) |>
  mutate(
  ...
  group_by(combo_id, run_id, tick,log_desertion_strictness,type) |>
  summarise(exp_value = sum(exp_value))

df_total <- exp_data |>
  group_by(combo_id, run_id, tick,log_desertion_strictness) |>
  summarise(exp_value = sum(exp_value)) |>
  mutate(type = "total")
...
exp_summary <- exp_data |>
  group_by(tick, log_desertion_strictness, type) |>
...
ggplot() +
  ...
  geom_line(data = exp_summary, 
            aes(x = tick, y = mean_exp_value, color = log_desertion_strictness), 
            linewidth = 1.2) +
  ...
  labs(x = "Years", 
       ...
       caption = cap_text,
       title = "Aggregated Expected Signal Value over time by Desertion Strictness") +
  theme(panel.background = element_rect(fill = "white", color = "black"),
        panel.grid.major.y = element_line(color = "grey"),
        panel.grid.minor.y = element_line(color = "lightgrey"),
        panel.grid.minor.x = element_line(color = NA),          
...
ggsave("./figures/log_desertion_strictness_agg_expected_value.png",
