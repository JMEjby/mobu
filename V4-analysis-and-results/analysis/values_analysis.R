...
count_data <-  data |>
  ...
  mutate(leader_values = factor(leader_values, labels = c("Value Profile A", "Value Profile B", "Value Profile C")),
         tick = (tick+1)/2)

count_summary <- count_data |>
  group_by(tick, leader_values) |>
...
cap_text <- paste("Simulations per combination:", max(count_data$run_id), 
                  ...
                  "| Leader election:", str_to_title(gsub(pattern = "-",  " ", count_data$leader_election[1])),
...
ggplot() +
  geom_line(data = count_data, 
            aes(x = tick, y = count_eas, 
                color = leader_values, 
                group = interaction(combo_id, run_id, leader_values)), 
            alpha = 0.2) +
  ...
  geom_line(data = count_summary, 
            aes(x = tick, y = mean_ea_count, color = leader_values), 
            linewidth = 1.2) +
  ...
  labs(x = "Years", 
       y = "Number of EAs",
       caption = cap_text,
       title = "Number of EAs over time by Leader Values") +
...
ggsave("./figures/leader_values_number_of_eas.png",
...
cap_text <- paste("Simulations per combination:", max(grad_count_data$run_id), 
                  ...
                  "| Leader election:", str_to_title(gsub(pattern = "-",  " ", grad_count_data$leader_election[1])),
...
ggplot() +
  ...
  geom_line(data = grad_count_summary, 
            aes(x = tick, y = mean_ea_year4, color = leader_values), 
            linewidth = 1.2) +
  ...
  labs(x = "Years (Semester 2 only)", 
       ...
       caption = cap_text,
       title = "Number of EAs Graduating at the end of each year by Leader Values") +
  theme(panel.background = element_rect(fill = "white", color = "black"),
        panel.grid.major.y = element_line(color = "grey"),
        panel.grid.minor.y = element_line(color = "lightgrey"),
        panel.grid.minor.x = element_line(color = NA),          
...
ggsave("./figures/leader_values_number_of_ea_grads.png",
...
cap_text <- paste("Simulations per combination:", max(exp_data$run_id), 
                  ...
                  "| Leader election:", str_to_title(gsub(pattern = "-",  " ", exp_data$leader_election[1])),
...
df_counts <- exp_data |>
  select(combo_id, run_id, tick, leader_values, count_eas, count_noneas) |>
  pivot_longer(
...
# Join the datasets
exp_data <- df_long |>
  left_join(df_counts, by = c("combo_id", "run_id", "tick","leader_values","type")) |>
  mutate(
  ...
  group_by(combo_id, run_id, tick,leader_values,type) |>
  summarise(exp_value = sum(exp_value))

df_total <- exp_data |>
  group_by(combo_id, run_id, tick,leader_values) |>
  summarise(exp_value = sum(exp_value)) |>
  mutate(type = "total")
...
exp_summary <- exp_data |>
  group_by(tick, leader_values, type) |>
...
ggplot() +
  ...
  geom_line(data = exp_summary, 
            aes(x = tick, y = mean_exp_value, color = leader_values), 
            linewidth = 1.2) +
  ...
  labs(x = "Years", 
       ...
       caption = cap_text,
       title = "Aggregated Expected Signal Value over time by Leader Values") +
  theme(panel.background = element_rect(fill = "white", color = "black"),
        panel.grid.major.y = element_line(color = "grey"),
        panel.grid.minor.y = element_line(color = "lightgrey"),
        panel.grid.minor.x = element_line(color = NA),          
...
ggsave("./figures/leader_values_agg_expected_value.png",
