...
count_data <-  data |>
  filter(social_influence == 0.5,
         ...
         leader_election == "external",
         ea_bias == 1.5
  )|>
  mutate(log_recruitment_strictness = factor(log_recruitment_strictness),
         tick = (tick+1)/2)

count_summary <- count_data |>
  group_by(tick, log_recruitment_strictness) |>
  summarise(mean_ea_count = mean(count_eas), 
            sd_ea_count = sd(count_eas), 
            .groups = "drop") |>
...
cap_text <- paste("Simulations per combination:", max(count_data$run_id), 
                  ...
                  "| Leader election method:", str_to_title(count_data$leader_election[1]),
...
ggplot() +
  geom_line(data = count_data, 
            aes(x = tick, y = count_eas, 
                color = log_recruitment_strictness, 
                group = interaction(combo_id, run_id, log_recruitment_strictness)), 
            alpha = 0.4) +
  geom_ribbon(data = count_summary, 
              aes(x = tick, 
                  ymin = ribbon_lower, 
                  ymax = ribbon_upper, 
                  fill = log_recruitment_strictness), 
              alpha = 0.3) +
  geom_line(data = count_summary, 
            aes(x = tick, y = mean_ea_count, color = log_recruitment_strictness), 
            linewidth = 1.2) +
  ...
  scale_color_brewer(type = "qual", palette = "Spectral", name = "Recruitment Strictness") +
  scale_fill_brewer(type = "qual", palette = "Spectral", name = "Recruitment Strictness") +
  labs(x = "Years", 
       y = "Number of EAs",
       caption = cap_text,
       title = "Number of EAs over time by Recruitment Strictness") +
...
ggsave("./figures/join_strictness_number_of_eas.png",
...
cap_text <- paste("Simulations per combination:", max(grad_count_data$run_id), 
                  ...
                  "| Leader election method:", str_to_title(grad_count_data$leader_election[1]),
...
ggplot() +
  ...
  geom_line(data = grad_count_summary, 
            aes(x = tick, y = mean_ea_year4, color = log_recruitment_strictness), 
            linewidth = 1.2) +
  ...
  labs(x = "Years (Semester 2 only)", 
       ...
       caption = cap_text,
       title = "Number of EAs Graduating at the end of each year by Recruitment Strictness") +
  theme(panel.background = element_rect(fill = "white", color = "black"),
        panel.grid.major.y = element_line(color = "grey"),
        panel.grid.minor.y = element_line(color = "lightgrey"),
        panel.grid.minor.x = element_line(color = NA),          
...
ggsave("./figures/join_strictness_number_of_ea_grads.png",
...
cap_text <- paste("Simulations per combination:", max(exp_data$run_id), 
                  ...
                  "| Leader election method:", str_to_title(exp_data$leader_election[1]),
...
df_counts <- exp_data |>
  select(combo_id, run_id, tick, log_recruitment_strictness, count_eas, count_noneas) |>
  pivot_longer(
...
# Join the datasets
exp_data <- df_long |>
  left_join(df_counts, by = c("combo_id", "run_id", "tick","log_recruitment_strictness","type")) |>
  mutate(
  ...
  group_by(combo_id, run_id, tick,log_recruitment_strictness,type) |>
  summarise(exp_value = sum(exp_value))

df_total <- exp_data |>
  group_by(combo_id, run_id, tick,log_recruitment_strictness) |>
  summarise(exp_value = sum(exp_value)) |>
  mutate(type = "total")
...
exp_summary <- exp_data |>
  group_by(tick, log_recruitment_strictness, type) |>
...
ggplot() +
  ...
  geom_line(data = exp_summary, 
            aes(x = tick, y = mean_exp_value, color = log_recruitment_strictness), 
            linewidth = 1.2) +
  ...
  labs(x = "Years", 
       ...
       caption = cap_text,
       title = "Aggregated Expected Signal Value over time by Recruitment Strictness") +
  theme(panel.background = element_rect(fill = "white", color = "black"),
        panel.grid.major.y = element_line(color = "grey"),
        panel.grid.minor.y = element_line(color = "lightgrey"),
        panel.grid.minor.x = element_line(color = NA),          
...
ggsave("./figures/log_recruitment_strictness_agg_expected_value.png",
