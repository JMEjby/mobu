...
ggplot() +
  ...
  labs(x = "Years", 
       y = "Number of EAs",
       caption = cap_text,
       title = "Number of EAs over time by Leader Influence") +
...
ggsave("./figures/relative_leader_influence_number_of_eas.png",
...
ggplot() +
  ...
  labs(x = "Years (Semester 2 only)", 
       ...
       caption = cap_text,
       title = "Number of EAs Graduating at the end of each year by Leader Influence") +
  theme(panel.background = element_rect(fill = "white", color = "black"),
        panel.grid.major.y = element_line(color = "grey"),
        panel.grid.minor.y = element_line(color = "lightgrey"),
        panel.grid.minor.x = element_line(color = NA),          
...
ggsave("./figures/relative_leader_influence_number_of_ea_grads.png",
...
df_counts <- exp_data |>
  select(combo_id, run_id, tick, relative_leader_influence, count_eas, count_noneas) |>
  pivot_longer(
...
# Join the datasets
exp_data <- df_long |>
  left_join(df_counts, by = c("combo_id", "run_id", "tick","relative_leader_influence","type")) |>
  mutate(
  ...
  group_by(combo_id, run_id, tick,relative_leader_influence,type) |>
  summarise(exp_value = sum(exp_value))
  
df_total <- exp_data |>
  group_by(combo_id, run_id, tick,relative_leader_influence) |>
  summarise(exp_value = sum(exp_value)) |>
  mutate(type = "total")
...
exp_summary <- exp_data |>
  group_by(tick, relative_leader_influence, type) |>
...
ggplot() +
  ...
  labs(x = "Years", 
       ...
       caption = cap_text,
       title = "Aggregated Expected Signal Value over time by Leader Influence") +
  theme(panel.background = element_rect(fill = "white", color = "black"),
        panel.grid.major.y = element_line(color = "grey"),
        panel.grid.minor.y = element_line(color = "lightgrey"),
        panel.grid.minor.x = element_line(color = NA),          
...
ggsave("./figures/relative_leader_influence_agg_expected_value.png",
