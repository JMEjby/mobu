...
to re-assign-groups
  ask turtles [
    set time-as-type time-as-type + 0.5
    set year year + 0.5
  ]

  set semester ifelse-value semester = 1 [ 2 ] [ 1 ]
...
to graduate-students
  ask eas with [ year > 4.5 ] [ die ]
  ask outsiders with [ year > 4.5 ] [ die ]

  ifelse leader-election = "external" [
    ask leaders with [ time-as-type > leader-lifespan ] [ die ]
  ] [
    ask leaders with [ year > 4.5 ] [ die ]
  ]
end

to induct-students
  let n-new-students n-agents - ( count turtles )

  create-outsiders n-new-students [
    setxy 0 0 ; temporary position
    set shape "circle" ; make outsiders circles
    set recruited? false ; not recruited
    set year 1 ; in year 1
    set time-as-type 1 ; starting as an outsider
    set value-profile value-category (random-normal pop-mean pop-sd)
    setup-signals
    create-links-with other turtles
  ]
end
